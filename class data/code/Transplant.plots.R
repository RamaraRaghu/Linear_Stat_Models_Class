...
donortype.plot <- function(d_matrix, title, Year = seq(1988, 2017, 1), labels = c("DD Trans.", "LD Trans.", "DD", "LD"), col = c("red", "blue", "green", "purple"))
{
	...
	legend(2017.5, ub-lb, legend = labels, col = col, lwd = 1, cex = .8)
...
region.plot <- function(M, title, Year = seq(1988, 2017, 1), labels = c("R11 Trans.", "R11 Donor.", "UVA", "UNC", "MCV", "Duke"), col =c("red", "blue", "orange", "lightblue3", "purple", "blue3"))
{
...
center.plot <- function(M, title, Year = seq(1988, 2017, 1), labels = c( "UVA", "UNC", "MCV", "Duke"), col =c( "orange", "lightblue3", "purple", "blue3"))
{
	lb <- min(apply(M, 2, min))
	ub <- max(apply(M, 2, max))
	par(xpd = T)
